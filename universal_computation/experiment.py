...
def run_experiment(
    ...
    # Changed because we can't use command line args
    if 'num_iters' not in exp_args:
        exp_args['num_iters'] = 10000
    if 'steps_per_iter' not in exp_args:
        exp_args['steps_per_iter'] = 100
    if 'test_steps_per_iter' not in exp_args:
        exp_args['test_steps_per_iter'] = 25
    if 'log_to_wandb' not in exp_args:
        exp_args['log_to_wandb'] = False
    if 'wandb_project' not in exp_args:
        exp_args['wandb_project'] = "my_project"
    if 'include_date' not in exp_args:
        exp_args['include_date'] = True
    if 'save_models' not in exp_args:
        exp_args['save_models'] = False
    if 'save_models_every' not in exp_args:
        exp_args['save_models_every'] = 25
    if 'device' not in exp_args:
        exp_args['device'] = 'cuda'
    if 'gpu_batch_size' not in exp_args:
        exp_args['gpu_batch_size'] = 16
